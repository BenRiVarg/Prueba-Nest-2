...
@section('content')
    ...
    <div>
          ...
          <div class="containter">
            ...
            <form action="{{route('crudM.store')}}" method="POST"  >
                                        @csrf
                                        <input type="hidden" name="manual" value="{{$id}}">
                                        <input type="hidden" name="nSeccion" value="{{$conglomerado['nomSecc']}}">
                                        @for($i=0;$i<=count($conglomerado['subS'])-1;$i++)
                                            @php
                                            $subS=$conglomerado['subS'][$i];
                                            $nomSubS="nombSub".$i;
                                            $bautCont="cont".$i;
                                            $bautComp="comp".$i;
                                            $SubSeccion_i=$SubSeccionesTemp[$i];
                                            @endphp

                                            <input type="hidden" name="{{$nomSubS}}" value="{{$subS->subSeccion}}">
                                            <input type="hidden" name="{{$bautCont}}" value="{{$subS->contador}}">
                                            <input type="hidden" name="{{$bautComp}}" value="{{$subS->compilado}}">
                                            <input type="hidden" name="contador" value="{{count($conglomerado['subS'])}}">
                                        @endfor
                                     
                                      <div class="pull-right">
                                        <button type="submit" class="btn btn-primary btn-border" style="margin-right:50px;">Aprobar</button>
                                      </div>
                                     </form>
        </div>
    </div>
